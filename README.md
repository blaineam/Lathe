...
**SVT-AV1** (BSD-3-Clause-Clear, plus the Alliance for Open Media Patent
License 1.0), for `LatheAV1`, because no Apple device encodes AV1. It is a
static binary target — permissive, so nothing requires it to be replaceable —
built from a pinned upstream tag by `Scripts/build-svtav1-xcframework.sh` and
published as a release asset. `Sources/LatheAV1/VENDORING.md` has the tag,
commit, slices and the two notices an app must carry.
